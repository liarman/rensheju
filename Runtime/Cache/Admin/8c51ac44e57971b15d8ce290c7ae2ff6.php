...
<table id="washGrid" class="easyui-datagrid" style="width:92%;height:800px" url="<?php echo U('Admin/Wash/ajaxWashList');?>" pagination="true"  toolbar="#toolbar" singleSelect="true" rownumbers="true">
    <thead>
    <tr>
...
<div id="toolbar">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="addWash()">添加</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editWash()">编辑</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="destroyWash()">删除</a>
</div>
<!-- 添加 -->
<div id="addWash" class="easyui-dialog" title="添加" style="width:800px;height:650px;padding:10px 20px;" closed="true" modal="true">
    <form id="addWashForm" method="post">
        <table>
            <tr>
                <td>商家名称:</td>
            ...
            <tr>
                <td></td>
                <td><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="addWashSubmit()" style="width:90px">保存</a>
                    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#addWash').dialog('close')" style="width:90px">取消</a></td>
            </tr>
        </table>

    </form>
</div>
<div id="editWash" class="easyui-dialog" title="编辑" style="width:800px;height:650px;padding:10px 20px;" closed="true" modal="true">
    <form id="editWashForm" method="post">
        ...
        <table>
            ...
            <tr>
                <td></td>
                <td><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="editWashSubmit()" style="width:90px">保存</a>
                    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#editWash').dialog('close')" style="width:90px">取消</a></td>
            </tr>
        </table>
...
<script type="text/javascript">
    var url;
    function addWash(){
        $('#addWash').dialog('open').dialog('setTitle','添加商家');
        $('#addWashForm').form('clear');
        var $radios = $('.addstatus');
        $radios.filter('[value=1]').prop('checked', true);
        url="<?php echo U('Admin/Wash/add');?>";
    }
    function addWashSubmit(){
        $('.addcontent').val(editor.html());
        $('#addWashForm').form('submit',{
            url: url,
            onSubmit: function(){
            ...
            success:function(data){
                ...
                if(data.status==1){
                    ...
                    $('#addWash').dialog('close');
                    $('#washGrid').datagrid('reload');
                }else {
    ...
    function editWashSubmit(){
        $('.editcontent').val(editor2.html());
        $('#editWashForm').form('submit',{
            url: url,
            onSubmit: function(){
                return $(this).form('validate');
            },
    ...
    //编辑会员对话窗
    function editWash(){
        var row = $('#washGrid').datagrid('getSelected');
        if(row==null){
            $.messager.alert('Warning',"请选择要编辑的行", 'info');return false;
        }
        editor2.html(row.content);
        if (row){
            $('#editWash').dialog('open').dialog('setTitle','编辑');
            $('#editWashForm').form('load',row);
            url ="<?php echo U('Admin/Wash/edit');?>"+'/id/'+row.id;
        }
    }
    function destroyWash(){
        ...
        if (row){
            $.messager.confirm('删除提示','真的要删除?',function(r){
                if (r){
                    var durl="<?php echo U('Admin/Wash/delete');?>";
                    $.getJSON(durl,{id:row.id},function(result){
                        if (result.status){
                            $('#washGrid').datagrid('reload');    // reload the user data
                        } else {
